...
def classify_video(
    ...
    # Decode video frames
    num_frames_decode = 64  # Decode enough frames for both pathways
    ...
    # Convert BGR → RGB if needed (PyAV already gives RGB, but just in case)
    # frames are already (T, H, W, 3) in RGB from decode_video_pyav
    ...
    slow_tensor, fast_tensor = pack_frames_slowfast(
        ...
        num_frames_fast=32,
        crop_size=224,
        spatial_transform=None,  # No augmentation for inference
    )

    # Add batch dimension and move to device
    slow_tensor = slow_tensor.unsqueeze(0).to(device)   # (1, 3, 8, 224, 224)
    fast_tensor = fast_tensor.unsqueeze(0).to(device)    # (1, 3, 32, 224, 224)

    # Run inference
    with torch.no_grad():
        logits = model([slow_tensor, fast_tensor])       # (1, num_classes)
    ...
    # Build results
    all_probs = {IDX_TO_CLASS[i]: float(probs[i]) for i in range(num_classes)}
...
def main():
    ...
    # Classify
    result = classify_video(args.video, args.checkpoint, device)
